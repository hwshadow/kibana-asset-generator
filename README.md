...
```sh
$ kag -conf="/etc/app.yaml" -template="/etc/"
```
#### Run dry with connection to elastic
```sh
$ kag -conf="/etc/app.yaml" -template="/etc/" -idx=".kibana"
```
#### Run with connection to elastic and writes enabled
```sh
$ kag -conf="/etc/app.yaml" -template="./kag_template/" -write=true -dashTitle="dash" -pre="testdata-dnd-" -idx="testdata.dnd" -timeField="dob"
```
